...
def send_cbu_history_to_mongo(report, verbose=True):
    ...
    try:
        ...
        ensure_unique_daily_index(
            collection,
            ("date_only", "bank_name", "currency"),
            "uniq_cbu_history_currency_day",
            "cbu_history",
            verbose=verbose,
        )
        operations = []
...
def build_collection_key_match(key_fields):
    return {
...
def build_partial_index_filter(key_fields):
    return {
        field: {"$exists": True}
        for field in key_fields
    }
...
def cleanup_duplicate_docs(collection, key_fields, label, verbose=True):
    pipeline = [
        {"$match": build_collection_key_match(key_fields)},
        {
            "$sort": {
                "date_only": -1,
                "timestamp": -1,
    ...
    for group in collection.aggregate(pipeline, allowDiskUse=True):
        ...
        delete_result = collection.delete_many({"_id": {"$in": stale_ids}})
        duplicate_groups += 1
        removed_docs += delete_result.deleted_count

    if verbose and removed_docs:
        print(
            f"[MongoDB][{label}] удалено дублей: {removed_docs} "
            f"(ключей: {duplicate_groups})"
        )

    return {"duplicate_groups": duplicate_groups, "removed_docs": removed_docs}


def ensure_unique_daily_index(collection, key_fields, index_name, label, verbose=True):
    cleanup_summary = cleanup_duplicate_docs(collection, key_fields, label, verbose=verbose)
    collection.create_index(
        [(field, 1) for field in key_fields],
        name=index_name,
        unique=True,
        partialFilterExpression=build_partial_index_filter(key_fields),
    )

    if verbose:
        print(f"[MongoDB][{label}] индекс {index_name} подтвержден")
...
def send_to_mongo(report, verbose=True):
    ...
    try:
        ...
        banks_collection = db[MONGO_COLLECTION_NAME]
        gold_collection = db[MONGO_GOLD_COLLECTION_NAME]
        predictions_collection = db[MONGO_PREDICTIONS_COLLECTION_NAME]

        ensure_unique_daily_index(
            banks_collection,
            ("date_only", "bank_name", "currency"),
            "uniq_daily_bank_currency",
            "banks",
            verbose=verbose,
        )
        ensure_unique_daily_index(
        ...
        ensure_unique_daily_index(
            ...
            ("date_only", "bank_name", "currency"),
            "uniq_daily_prediction_currency",
            "predictions",
            verbose=verbose,
        )
        summary = {
        ...
        if bank_operations:
            result = banks_collection.bulk_write(bank_operations, ordered=False)
            summary["banks"] = {
                "status": "success",
                "inserted": result.upserted_count,
                "updated": result.modified_count,
        ...
        if gold_operations:
            result = gold_collection.bulk_write(gold_operations, ordered=False)
            summary["gold"] = {
                "status": "success",
                "inserted": result.upserted_count,
                "updated": result.modified_count,
        ...
        if prediction_operations:
            result = predictions_collection.bulk_write(
                prediction_operations,
                ordered=False,
            )
            summary["predictions"] = {
